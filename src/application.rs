...
mod imp {
    ...
    impl gio::subclass::prelude::ApplicationImpl for ExampleApplication {
        fn activate(&self, app: &Self::Type) {
            ...
            app.set_resource_base_path(Some("/rss/videplayer/"));
            app.setup_css();
...
impl ExampleApplication {
    ...
    fn setup_css(&self) {
        ...
        provider.load_from_resource("/rss/videplayer/style.css");
    ...
    fn show_about_dialog(&self) {
        let dialog = gtk::AboutDialogBuilder::new()
            .program_name("RSS Video Player")
            .logo_icon_name(config::APP_ID)
            // Insert your license of choice here
            // .license_type(gtk::License::MitX11)
            .website("https://github.com/paulskinau/rss-videoplayer.git/")
            .version(config::VERSION)
            .transient_for(&self.get_main_window())
            .modal(true)
            .authors(vec!["Paul Skinner".into()])
            .artists(vec!["Paul Skinner".into()])
            .build();
    ...
    pub fn run(&self) {
        info!("RSS Video Player ({})", config::APP_ID);
